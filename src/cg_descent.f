c Modified cg_descent source files downloaded from
c    http://www.math.ufl.edu/~hager/papers/CG/Archive/
c to take extra arguments and pass to the user defined functions.
c -- by Meeyoung Park
...
c initial function and gradient evaluations, initial direction
      ...
      call cg_value (f, x, n, z, lenz, nobs, lam)
      nf = nf + 1
      call cg_grad (g, x, n, z, lenz, nobs, lam)
      ng = ng + 1
      f0 = f + f
      gnorm = zero
      xnorm = zero
      ...
      do iter = 1, maxit
          ...
          if ( QuadStep ) then               
              ...
              if ( t .gt. QuadCutOff ) then
                  ...
                  call cg_value (ftemp, xtemp, n, z, lenz, nobs, lam)
                  nf = nf + 1
                  if ( ftemp .lt. f ) then
...
c parameters in Wolfe and approximiate Wolfe conditions, and in update
     ...
     &                     cg_value, cg_grad, z, lenz, nobs, lam)
          else
